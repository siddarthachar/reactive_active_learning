...
def relax_atoms(atoms, dp_path, pb_name='graph.pb', logpath='rlx.out'):
...
#                     print(react_bonds[0] == initial_bonds[0])
                    if react_bonds[0] != initial_bonds[0]: # if the initial and final are NOT the same. 
                        ...
                        relaxed = relax_atoms(last, dp_path, 'graph.pb') # relaxing the final image from SE-GSM
                        ...
                        rlx_interm += [relaxed]
                        _,unp = get_multiplicity({'C':4,'H':1,'N':5,'O':6},relaxed)
                        ...
                        if isnotradical :
                            ...
                            maintain_order_poscar(packed_ase, f'struct.packed/{str(struct_count).zfill(3)}.poscar', ['C','H','N','O'])
