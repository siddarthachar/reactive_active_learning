...
atom_e = {'C': -1.2782520, 'H':-1.11103621, 'N':-3.1086907, 'O':-1.5480695}

deepmd_paths = []
main_path=os.getcwd()
data_list=sorted([x for x in os.listdir(f'{main_path}') if x.split('.')[0]=='md'])
...
for i, d in enumerate(data_list):
    ...
    image_list = sorted([x for x in os.listdir(data_path) if 'deepmd_data' not in x])
    ...
    for j, im in enumerate(image_list):
        ...
        scf_test = int(os.popen(f'grep DAV: {image_path}/OSZICAR').read().split('\n')[-2].split()[1]) < 200 # less than 200 scf steps
        mag_read = os.popen(f'grep F= {image_path}/OSZICAR').read().split()[-1]
        mag_int_check = np.abs(float(mag_read) - int(float(mag_read))) < 1e-3 # checking if total magnetic moment is within a given threshold or not.
        ...
        # print(float(os.popen(f'grep F= {image_path}/OSZICAR').read().split()[-1]))
        if (contcar_test + scf_test + atomization_energy_check + mag_int_check) == 4: # if all of them are true
            print(image_path)
            a=read(f'{image_path}/OUTCAR')
            outcars += [a]
    print('total outcars: ', len(outcars))
    if len(outcars):
        box = []
        coord = []
        energy = []
