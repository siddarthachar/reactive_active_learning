...
def prod_adj_reorder_H(react, prod_adjacency):
...
reactant_smiles = ['C=N','O']
add_reactant_smiles = ['C=N','O'] # if the reactants do not satisfy the products, then we can use these set of reactants to statisfy the stoichiometry.
...
product_smiles = ['CN','NCO','C=O','[H][H]','CNO']
prod_mol = [Chem.rdmolops.AddHs(Chem.MolFromSmiles(rx)) for rx in product_smiles]
...
#                 more_react = reactant_smiles.copy()
        ...
        prod_new=prod_adj_reorder_H(react_sym_i, prod_adj_i) 
        commands = generate_add_break_commands(react_adj_list_i, prod_new)
...
dp_path='/bgfs/kjohnson/pbs13/DeePMD/reactive_active_learning/methyle_03/new_DP0/ral.self_constistent/gen-2/train/dp2/graph.pb'
...
filepath1 = f"{working_path}/ch2nh-h2o.xyz"
reactant1=read(filepath1)
filepath2 = f"{working_path}/ch2nh-h2o.xyz"
