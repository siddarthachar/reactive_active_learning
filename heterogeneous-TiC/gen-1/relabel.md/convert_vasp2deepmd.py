...
atom_e = {'C':-1.27825, 'H':-1.11036, 'Ti': -2.29936}
...
for i, d in enumerate(data_list):
    ...
    for j, im in enumerate(image_list):
        ...
        # tests
        contcar_test = Path(f'{image_path}/CONTCAR').is_file()
        ...
        # print(float(os.popen(f'grep F= {image_path}/OSZICAR').read().split()[-1]))
        if (contcar_test + scf_test + atomization_energy_check) == 3: # if all of them are true
            print(image_path)
