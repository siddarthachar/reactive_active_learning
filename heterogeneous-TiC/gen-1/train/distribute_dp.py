...
main_path = '/bgfs/kjohnson/ska31/6AA/reactive_active_learning/TiC-methane-coupling/'
...
#	levels = [x for x in sorted(os.listdir(f'../../gen-{current_gen-1}/relabel/')) if x.split('.')[0] == 'level'] # use levels only for SE_GSM or reactions - may have to chage later
	# deepmd_data_curr = []
	deepmd_data_curr = os.popen(f'cat ../../gen-{current_gen-1}/fp/deepmd_data_path.dat').read().split('\n')[:-1]
	# for level in levels:
