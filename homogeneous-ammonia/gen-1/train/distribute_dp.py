...
main_path = '/bgfs/kjohnson/ska31/6AA/reactive_active_learning/ANH/ral-intermediates.02'
...
for i in range(4):
	...
	levels = [x for x in sorted(os.listdir(f'../../gen-{current_gen-1}/relabel.gsm/')) if x.split('.')[0] == 'level']
	deepmd_data_curr = []

	for level in levels:
		deepmd_data_curr += os.popen(f'cat ../../gen-{current_gen-1}/relabel.gsm/{level}/deepmd_data_path.dat').read().split('\n')[:-1]

	# including relabeled md configurations
	deepmd_data_curr += os.popen(f'cat ../../gen-{current_gen-1}/relabel.md/deepmd_data_path.dat').read().split('\n')[:-1]
	...
	os.chdir(f'dp{i}')
	os.system(f'cp {main_path}/gen-{current_gen-1}/train/dp{i}/job.slurm .')
	job_init_model_train = f'dp train input.json --init-model {main_path}/gen-{current_gen-1}/train/dp{i}/model.ckpt'
